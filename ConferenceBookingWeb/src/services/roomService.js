...
/**
 * Fetch ALL rooms for management (active + inactive)
 * @returns {Promise<Array>} Array of all room objects
 */
export const fetchAllRoomsManagement = async () => {
  try {
    // Fetch active AND inactive rooms separately and merge.
    // This works even with older API versions that default isActive=true.
    ...
    console.error('❌ Failed to fetch rooms for management:', error);
    throw error;
  }
};
